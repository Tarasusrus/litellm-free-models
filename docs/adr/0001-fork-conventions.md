...
| File | Edit | Why |
|---|---|---|
| `docker-compose.yaml` | proxy service: official image, repo mounted read-only at `/repo`, `entrypoint: fork/docker-entrypoint.sh`; container names and host port overridable from `.env`; the `settings-ui` service (loopback only, repo read-write for the `.env` replace, Docker socket read-only); the `env-init` service (`fork/ensure_secrets.py`), `postgres` reading `POSTGRES_PASSWORD_FILE` and `redis`'s command replaced by a shell wrapper reading its password file, both `depends_on: env-init: condition: service_completed_successfully`, and the `secrets-data` volume they share; `litellm-proxy`'s `DATABASE_URL`/`REDIS_PASSWORD` no longer `:?`-required, since `env-init` cannot have run yet when compose interpolates them | one-command start; compose has no other hook for "render before start" that survives a missing `config.yaml` (a bind mount of a missing file creates a directory); compose interpolates every `${VAR}` once, at the start of `up`, so a password generated during that same `up` can only reach `postgres`/`redis` through a file their own entrypoint reads at container start, never through `${POSTGRES_PASSWORD}`/`${REDIS_PASSWORD}` |
